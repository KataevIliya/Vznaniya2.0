...
# Есть и другие способы, но вы из не используйте (мне их закрывать лень), ок?
with Proxy() as proxy:
    # Эту строчку расскомменть, если хочешь убрать окошечно браузера
    # Но я этого делать не рекомендую, так как не проверял, будет ли так работать
    # Теперь проверял... не будет решать "послушай" и тест.
    ...
    # Поолучаем модуль с заданным именем
    ...
    # Немного отлпдочных данных... которые вам не нужны
    # pprint(mod.data)
